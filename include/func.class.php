...
// 	echo "SELECT * FROM `#@__infolist` WHERE classid=$cid AND delstate='' AND checkinfo=true ".$f." ORDER BY orderid,id DESC LIMIT 0,$num";
	$dosql->Execute("SELECT * FROM `#@__infolist` WHERE classid=$cid AND delstate='' ".$f." ORDER BY flag DESC,posttime DESC LIMIT $start,$num");
	while($row = $dosql->GetArray())
	{
		...
		if ($timectrl==1){
			...
			$contstr.="<li><a href='".$gourl."' ><img src='".$row['picurl']."' width='460' height='240' alt='".$row['title']."'></a></li>";
		}elseif($timectrl==3){
	...
	if(empty($contstr)){
		$dosql->Execute("SELECT * FROM `#@__infolist` WHERE parentid=$cid AND delstate=''  ".$f." ORDER BY flag DESC,posttime DESC LIMIT 0,$num");
		while($row = $dosql->GetArray())
		{
...
//  	echo "SELECT * FROM `#@__infolist` WHERE classid=$cid AND delstate='' AND checkinfo=true ".$f." ORDER BY orderid,id DESC LIMIT 0,1";
	$row =$dosql->GetOne("SELECT * FROM `#@__infolist` WHERE classid=$cid AND delstate='' AND checkinfo=true ".$f." ORDER BY posttime DESC LIMIT 0,1");
	if(!empty($row)){
	if($row['linkurl'] == '') $gourl = 'newsshow.php?cid='.$row['classid'].'&id='.$row['id'];
		else $gourl = $row['linkurl'];
			$contstr ="<img src=".$row['picurl']." width='109' height='72'>
                    <div class='pull-left center-box-info'>
                        <h1><a href='".$gourl."'>". ReStrLen($row['title'],19)."</a></h1>		
                        <span>". ReStrLen(ClearHtml($row['description']),50)."</span>
                    </div>";
	}
	return $contstr;
}
	...
	if($row['linkurl'] == '') $gourl = 'newsshow.php?cid='.$row['classid'].'&id='.$row['id'];
			...
			$contstr ="
                    <div class='news-box-info'>
                        ...
                        <span>". ReStrLen(ClearHtml($row['content']),75)."</span>
                    </div>";
	}
	return $contstr;
}
...
function GetCatNameEn($cid=0)
{
	global $dosql;
 	//echo "SELECT `classname` FROM `#@__infoclass` WHERE `id`=$cid";
	$r = $dosql->GetOne("SELECT `keywords`,`parentid` FROM `#@__infoclass` WHERE `id`=$cid");
	
	if($r['parentid']){
		$r =$dosql->GetOne("SELECT `keywords` FROM `#@__infoclass` WHERE `id`=".$r['parentid']."");
	}
	if(isset($r['keywords']))
		return $r['keywords'];
	else
				...
				foreach($pid_arr as $v)
				{
					if(!empty($v))
					{
						...
						if(!empty($r['linkurl']))
							...
							$pos_str .= $sign.'<a href="'.$m.'?cid='.$r['id'].'">'.$r['classname'].'</a>';
					}
				}
			}
				...
				else
				{
					if(!empty($r['linkurl']))
						...
						switch ($r['infotype'])
							...
							echo $pos_str .= $sign.'<a href="'.$m.'?cid='.$r['id'].'">'.$r['classname'].'</a>';
				}
			}
			else
			{
...
function GetYdclass($id)
{
	...
	while($row = $dosql->GetArray($id))
	{
		...
		switch ($row['infotype'])
		{
			...
			$str .= '<li class="clearfix"><div class="yd-img"><a href="'.$m.'?cid='.$row['id'].'"><img src="'.$row['picurl'].'" width="105" height="105"/></a></div>';
			$str .= '<div class="yd-info"><a href="'.$m.'?cid='.$row['id'].'" class="yd-name">'.$classname.'</a><a href="'.$m.'?cid='.$row['id'].'" class="yd-btn">订购流程</a><a href="'.$m.'?cid='.$row['id'].'" class="yd-btn">订购流程</a></div>';
		...
		if($row!="")
			switch($row['admode']){
				case $row['admode']=='image':
					$str .= '<a href="'.$row['linkurl'].'" target="_blank"><img src="'.$row['picurl'].'"></a>';
					break;
				...
				<embed width="225" height="185" border="0" showdisplay="0" showcontrols="1" autostart="1" autorewind="0" playcount="0" moviewindowheight="225" moviewindowwidth="185" filename="'.$row['picurl'].'" src="'.$row['picurl'].'"></embed>
					...
					break;
				default:
					$str .= '<a href="'.$row['linkurl'].'" target="_blank">'.$row['adtext'].'</a>';
					break;
		}
	}
	return $str;
