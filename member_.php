...
//		}
		else
		{
			...
			$row = $dosql->GetOne("SELECT `id`,`password`,`enteruser`,`logintime`,`loginip`,`expval` FROM `#@__member` WHERE `username`='$username'");
			...
			else
			{
				...
				setcookie('enteruser',      AuthCode($row['enteruser'],'ENCODE'), $cookie_time);
				setcookie('lastlogintime', AuthCode($row['logintime'],'ENCODE'), $cookie_time);
				setcookie('lastloginip',   AuthCode($row['loginip']  ,'ENCODE'), $cookie_time);
...
else if($a == 'reg')
{
	...
	$cnname      = empty($cnname)      ? '' : $cnname;//昵称
	$enname      = empty($enname)      ? '' : $enname;//单位名称
	$qqnum      = empty($qqnum)      ? '' : $qqnum;//部门
	$telephone      = empty($telephone)      ? '' : $telephone;//职务
	$mobile      = empty($mobile)      ? '' : $mobile;//职务
	$validate   = empty($validate)   ? '' : strtolower($validate);
	...
	$sql = "INSERT INTO `#@__member` (username, password, email,cnname,enname,qqnum,telephone,mobile, expval, regtime, regip, logintime, loginip) VALUES ('$username', '$password','$cnname','$enname','$qqnum','$telephone','$mobile', '$email', '10', '$regtime', '$regip', '$regtime', '$regip')";
...
else if($a == 'saveedit')
{
	...
	if($password != '')
	{
		...
		$sql .= "password='$password'WHERE id='$id' AND `username`='$c_uname'	";
		if($dosql->ExecNoneQuery($sql))
		{
			ShowMsg('会员密码更改成功！','?c=edit');
			exit();
		}
	}else{
...
{
	require_once(PHPMYWIND_TEMP.'/default/member/edit2.php');
	exit();
}
