...
<body>
<div class="topToolbar"> <span class="title">有奖征集</span> <a href="javascript:location.reload();" class="reload">刷新</a></div>
...
<?php
				if(!empty($keyword))
				{
					...
					$dopage->GetPage("SELECT * FROM `#@__youjiang` where canshu = 1 ");
				}
				else
				{
					$dopage->GetPage("SELECT * FROM `#@__youjiang` where canshu = 1 ");
...
<td height="30" align="center"><a href="member11.php?id=<?php echo $row['id']; ?>">查看</a>&nbsp;/&nbsp;<a href="member12.php?id=<?php echo $row['id']; ?>">删除</a></td>
</tr><?php
				}
				?>
</table>
