...
if(isset($_GET['a']) and $_GET['a']=='del'){
	$dosql->ExecNoneQuery("DELETE FROM `#@__fabiao` WHERE  id=$id");
	ShowMsg('删除成功！','memberfb.php');
	exit();
}
?>
...
<body>
<div class="topToolbar"> <span class="title">论文发表</span> <a href="javascript:location.reload();" class="reload">刷新</a></div>
...
<td height="30" align="center"><strong>单位</strong></td>
<td height="30" align="center"><strong>部门</strong></td>
<td height="30" align="center"><strong>职务</strong></td>
<td height="30" align="center"><strong>时间</strong></td>
...
<?php
				if(!empty($keyword))
				{
					...
					$dopage->GetPage("SELECT * FROM `#@__fabiao` ");
				}
				else
				{
					$dopage->GetPage("SELECT * FROM `#@__fabiao` ");
...
<td height="30" align="center"><a href="memberfb1.php?id=<?php echo $row['id']; ?>">查看</a>&nbsp;/&nbsp;<a href="memberfb.php?a=del&id=<?php echo $row['id']; ?>">删除</a></td>
</tr><?php
				}
				?>
</table>
