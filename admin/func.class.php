 ...
 */
function Infolist($cid=0, $num=0,$timectrl=0,$flag='')
{
	global $dosql;
	$contstr = '';
	$f = '';
...
// 	echo "SELECT * FROM `#@__infolist` WHERE classid=$cid AND delstate='' AND checkinfo=true ".$f." ORDER BY orderid,id DESC LIMIT 0,$num";
	$dosql->Execute("SELECT * FROM `#@__infolist` WHERE classid=$cid AND delstate='' AND checkinfo=true ".$f." ORDER BY orderid,id DESC LIMIT 0,$num");
	while($row = $dosql->GetArray())
	{
		...
		if ($timectrl==1){
			$contstr.="<li><span>".GetDateMk($row['posttime'])."</span><a href='".$gourl."' >". ReStrLen($row['title'],14)."</a></li>";
		}elseif($timectrl==2){
			$contstr.="<li><a href='".$gourl."' ><img src='".$row['picurl']."' width='415' height='240' alt='".$row['title']."'></a></li>";
		}else{
			$contstr.="<li><a href='".$gourl."' >". ReStrLen($row['title'],14)."</a></li>";
		}
	}
	if(empty($contstr)){
		$dosql->Execute("SELECT * FROM `#@__infolist` WHERE parentid=$cid AND delstate='' AND checkinfo=true ".$f." ORDER BY orderid,id DESC LIMIT 0,$num");
		while($row = $dosql->GetArray())
		{
...
// 	echo "SELECT * FROM `#@__infolist` WHERE classid=$cid AND delstate='' AND checkinfo=true ".$f." ORDER BY orderid,id DESC LIMIT 0,1";
	$row =$dosql->GetOne("SELECT * FROM `#@__infolist` WHERE classid=$cid AND delstate='' AND checkinfo=true ".$f." ORDER BY orderid,id DESC LIMIT 0,1");
	if(!empty($row)){
	if($row['linkurl'] == '') $gourl = 'newsshow.php?cid='.$row['classid'].'&id='.$row['id'];
		else $gourl = $row['linkurl'];
			$contstr ="<img src=".$row['picurl']." width='109' height='72'>
                    <div class='pull-left center-box-info'>
                        ...
                        <span>". ReStrLen($row['content'],200)."</span>
                    </div>";
	}
	return $contstr;
}
...
{
	global $dosql;
// echo "SELECT `picurl` FROM `#@__info` WHERE `classid`=$cid";
...
function Getclass($id,$sas=2)
{
	...
	$row = $dosql->GetOne("SELECT * FROM `#@__infoclass` WHERE id=$id AND checkinfo=true ORDER BY orderid ASC");
	switch ($row['infotype'])
		{
			case 0:
				$m = 'about.php';
